...
//     }
//     .tag-admin { background: rgba(199, 235, 239, 1); color: var(--gold); border: 1px solid rgba(201,148,58,0.3); }
//     .tag-librarian { background: rgba(199, 235, 239, 1); color: #6dba85; border: 1px solid rgba(74,124,89,0.3); }
//     .tag-member { background:rgba(199, 235, 239, 1); color: #90adc0; border: 1px solid rgba(44,62,80,0.6); }
//     .tag-verified { background: rgba(199, 235, 239, 1); color: #6dba85; border: 1px solid rgba(74,124,89,0.25); }
//     .tag-pending { background: rgba(199, 235, 239, 1); color: #e07050; border: 1px solid rgba(181,69,27,0.25); }
...
//     .nav-item.active { background:rgba(255,255,255,0.04); border-color: rgba(201,148,58,0.2); color: var(--gold); }
...
//           Manage Your<br /><span>Library</span> Smarter
//         </h1>
//         <p className="hero-sub">
//           A complete platform for admins, librarians, and members to manage books,
//           memberships, and library operations effortlessly.
//         </p>
...
//   // After register: if MEMBER → go to OTP page. Else → show pending message.
...
//      // ALL roles now go to OTP page first
// // We save the email so OTP page knows where to send verification
// sessionStorage.setItem("otpEmail", form.email);
// sessionStorage.setItem("otpRole", form.role); // save role too
...
//             {form.role === "MEMBER" && (
//               <p style={{ fontSize: 12, color: "rgba(250,248,243,0.4)", lineHeight: 1.5 }}>
//                 📧 An OTP will be sent to your email to verify your account.
//               </p>
//             )}
//             {(form.role === "ADMIN" || form.role === "LIBRARIAN") && (
//               <p style={{ fontSize: 12, color: "rgba(250,248,243,0.4)", lineHeight: 1.5 }}>
//                 ⏳ Your account will require Super Admin approval before you can log in.
//               </p>
//             )}

//             <button className="btn-gold" type="submit" disabled={loading} style={{ marginTop: 8, width: "100%" }}>
...
//         </p>
//         <p style={{ textAlign: "center", fontSize: 13, color: "rgba(250,248,243,0.35)", marginTop: 8, cursor: "pointer" }} onClick={() => navigate("landing")}>
//           ← Back to Home
//         </p>
//       </div>
//     </div>
...
//     try {
//       const res = await fetch(`${API}/auth/verify?email=${encodeURIComponent(email)}&otp=${otpStr}`, {
//         method: "POST",
//       });
//       const txt = await res.text();
...
//     } else {
//         setSuccess("✅ Email verified! Please wait for Admin approval before logging in.");
//     }
...
//                     fontSize: 12
//                 }}>
//                     ⏳ After verifying your email, your account will also need 
//                     <strong> Admin approval</strong> before you can login.
//                 </span>
//             )}
//         </p>
//     );
...
//               ))}
//             </div>
//             <button className="btn-gold" onClick={handleVerify} disabled={loading} style={{ width: "100%", padding: "14px" }}>
...
//         {success && (
//           <button className="btn-gold" onClick={() => navigate("login")} style={{ width: "100%", padding: "14px", marginTop: 16 }}>
//             Go to Login →
//           </button>
//         )}

//         <p style={{ marginTop: 20, fontSize: 12, color: "rgba(250,248,243,0.35)" }}>
...
//       onLogin(user); // save user, navigate to dashboard
//     } catch (err) {
//       setError(err.message);
//     } finally {
//       setLoading(false);
...
//     setLoading(true);
//     try {
//       // This endpoint doesn't exist yet in your backend — placeholder
...
//     } catch (err) {
//       setError(err.message || "Failed to update password. (Add /api/auth/change-password to backend)");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//         <h1>My Profile</h1>
//         <p>View your account info and update your password</p>
//       </div>
...
//             { label: "Account Status", value: user.approved ? "Approved" : "Pending Approval" },
//           ].map((row, i) => (
//             <div key={i}>
...
//             <input type="password" placeholder="Enter current password" value={pwForm.currentPassword} onChange={e => setPwForm({ ...pwForm, currentPassword: e.target.value })} required />
//           </div>
//           <div className="form-group">
//             <label>New Password</label>
//             <input type="password" placeholder="Enter new password" value={pwForm.newPassword} onChange={e => setPwForm({ ...pwForm, newPassword: e.target.value })} required />
//           </div>
//           <div className="form-group">
...
//           </div>
//           <button className="btn-gold" type="submit" disabled={loading} style={{ alignSelf: "flex-start", minWidth: 160 }}>
//             {loading ? <Spinner /> : "Update Password"}
//           </button>
//         </form>
//         <p style={{ marginTop: 14, fontSize: 11, color: "rgba(250,248,243,0.3)" }}>
//           💡 Note: The change-password endpoint needs to be added to your Spring Boot backend.
//         </p>
//       </div>
//     </div>
//   );
...
//     } catch { /* backend endpoint may not exist yet */ }
...
//       fetchAll();      // also refresh all users list
//     } catch { 
...
//                 <button className="btn-gold" onClick={() => setActiveTab("pending")}>Review Pending Users</button>
//                 <button className="btn-outline" onClick={() => setActiveTab("users")}>View All Users</button>
//               </div>
//             </div>
//             <div className="card" style={{ marginTop: 20 }}>
//               <h3 style={{ marginBottom: 8 }}>ℹ️ Backend Integration Notes</h3>
//               <p style={{ fontSize: 13, color: "rgba(250,248,243,0.5)", lineHeight: 1.7 }}>
//                 The following API endpoints need to be added to your Spring Boot backend to make this dashboard fully functional:<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>GET /api/admin/pending-users</code> — list unverified/unapproved users<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>GET /api/admin/all-users</code> — list all users<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>POST /api/admin/approve?email=...</code> — approve a user<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>POST /api/auth/change-password</code> — update password
//               </p>
//             </div>
//           </div>
//         )}
...
//               <p>Review and approve Librarian / Admin accounts</p>
//             </div>
...
//                   <p style={{ fontSize: 12, marginTop: 4 }}>Add <code>/api/admin/pending-users</code> endpoint to your backend</p>
//                 </div>
//               ) : (
//                 <table>
...
//                         <td>
//                           <button className="btn-success" onClick={() => approveUser(u.email)}>Approve</button>
//                         </td>
//                       </tr>
//                     ))}
//                   </tbody>
...
//                   <div style={{ fontSize: 40, marginBottom: 12 }}>👥</div>
//                   <p>No users found</p>
//                   <p style={{ fontSize: 12, marginTop: 4 }}>Add <code>/api/admin/all-users</code> endpoint to your backend</p>
//                 </div>
//               ) : (
//                 <table>
...
//             <div className="user-chip-info">
//               <div className="user-chip-name">{user.name}</div>
//               <div className="user-chip-role">Librarian</div>
//             </div>
//           </div>
//           <button className="btn-outline" onClick={onLogout} style={{ width: "100%" }}>Logout</button>
//         </div>
...
//               <p>Manage books and track borrowings</p>
//             </div>
//             <div className="stats-grid">
//               {[
//                 { num: "—", label: "Total Books" },
//                 { num: "—", label: "Borrowed" },
//                 { num: "—", label: "Available" },
//                 { num: "—", label: "Overdue" },
//               ].map((s, i) => (
...
//             </div>
//             <div className="card">
//               <h3 style={{ marginBottom: 8 }}>ℹ️ What to Build Next</h3>
//               <p style={{ fontSize: 13, color: "rgba(250,248,243,0.5)", lineHeight: 1.8 }}>
//                 Add these endpoints to your Spring Boot backend to power this dashboard:<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>GET /api/books</code> — list all books<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>POST /api/books</code> — add a book<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>DELETE /api/books/{"{id}"}</code> — remove a book<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>GET /api/borrowings</code> — list borrowings<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>POST /api/borrowings/issue</code> — issue a book<br/>
//                 <code style={{ color: "var(--gold-light)", fontSize: 12 }}>POST /api/borrowings/return/{"{id}"}</code> — return a book
//               </p>
//             </div>
//           </div>
//         )}
...
//             <div className="page-header">
//               <h1>Manage Books</h1>
//               <p>Add, edit, and remove library books</p>
//             </div>
//             <div className="card" style={{ textAlign: "center", padding: 60 }}>
//               <div style={{ fontSize: 48, marginBottom: 16 }}>📖</div>
//               <h3 style={{ marginBottom: 8 }}>Books Module Coming Soon</h3>
//               <p style={{ fontSize: 13, color: "rgba(250,248,243,0.4)" }}>
//                 Once you add the Book entity and controller in your backend,<br />
//                 connect it here by calling <code style={{ color: "var(--gold-light)" }}>GET /api/books</code>
//               </p>
//             </div>
//           </div>
//         )}
...
//               <p style={{ fontSize: 13, color: "rgba(250,248,243,0.4)" }}>
//                 Add a Borrowing entity in your backend, then fetch data here.
//               </p>
//             </div>
//           </div>
//         )}
...
//             <div className="user-chip-info">
//               <div className="user-chip-name">{user.name}</div>
//               <div className="user-chip-role">Member</div>
//             </div>
//           </div>
//           <button className="btn-outline" onClick={onLogout} style={{ width: "100%" }}>Logout</button>
//         </div>
...
//               <p>Welcome to your library member portal</p>
//             </div>
...
//             <div className="stats-grid">
//               {[
//                 { num: "—", label: "Books Borrowed" },
//                 { num: "—", label: "Books Returned" },
//                 { num: "—", label: "Currently Reading" },
//                 { num: "—", label: "Overdue" },
//               ].map((s, i) => (
//                 <div className="stat-card" key={i}>
//                   <div className="stat-number">{s.num}</div>
...
//             <div className="card">
//               <h3 style={{ marginBottom: 8 }}>ℹ️ What's Next</h3>
//               <p style={{ fontSize: 13, color: "rgba(250,248,243,0.5)", lineHeight: 1.8 }}>
//                 Once your backend has Book and Borrowing endpoints, members can:<br/>
//                 • Browse available books<br/>
//                 • Request to borrow a book<br/>
//                 • View their borrowing history<br/>
//                 • Get notified about due dates
//               </p>
//             </div>
//           </div>
//         )}
...
//             <div className="page-header">
//               <h1>Browse Books</h1>
//               <p>Find and borrow books from our library</p>
//             </div>
//             <div className="card" style={{ textAlign: "center", padding: 60 }}>
//               <div style={{ fontSize: 48, marginBottom: 16 }}>🔍</div>
//               <h3 style={{ marginBottom: 8 }}>Book Catalog Coming Soon</h3>
//               <p style={{ fontSize: 13, color: "rgba(250,248,243,0.4)" }}>
//                 Add the Book entity to your backend, then call <code style={{ color: "var(--gold-light)" }}>GET /api/books</code> to display books here.
//               </p>
//             </div>
//           </div>
//         )}
...
//             <div className="page-header">
//               <h1>My Borrowings</h1>
//               <p>Your borrowing history and current books</p>
//             </div>
//             <div className="card" style={{ textAlign: "center", padding: 60 }}>
//               <div style={{ fontSize: 48, marginBottom: 16 }}>📋</div>
...
// // ─────────────────────────────────────────────
// // ROOT APP — handles routing + auth state
// // ─────────────────────────────────────────────
