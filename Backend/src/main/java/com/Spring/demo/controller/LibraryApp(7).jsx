...
// ── RenewButton — small inline button for renewing a single borrowing ────────
function RenewButton({ borrowingId, onSuccess }) {
  ...
  const handleRenew = async () => {
    ...
    try {
      ...
      const r = await fetch(
        `http://localhost:8080/api/borrow/borrowings/${borrowingId}/renew`,
      ...
      if (r.ok) {
        ...
        setErr(text.includes("ALREADY_RENEWED") ? "Already renewed" :
               text.includes("OVERDUE")         ? "Can't renew overdue" : "Failed");
      }
    } catch { setErr("Error"); }
                  ...
                  {currentBooks.map((b, i) => {
                    ...
                    return (
                      <tr key={i}>
                        ...
                        {/* Renew button — only for BORROWED (not OVERDUE), only if not already renewed */}
                        <td>
                          {b.renewed ? (
                            <span style={{ fontSize:11, color:"var(--text3)" }}>Renewed ✓</span>
                          ) : b.status === "OVERDUE" ? (
                            <span style={{ fontSize:11, color:"#F87171" }}>Can't renew</span>
                          ) : (
                            <RenewButton borrowingId={b.id} onSuccess={fetchBorrowings} />
                          )}
                        </td>
                      </tr>
                    );
